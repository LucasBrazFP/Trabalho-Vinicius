...
def ordenar_numeros():
    ...
    if not numeros:
        print("Arquivo vazio. Nada para ordenar.")
        return

    print(f"Ordenando números com paralelismo ({cpu_count()} núcleos)...")
    inicio = time.time()

    
    num_processos = min(cpu_count(), len(numeros))
    if num_processos <= 0:
        num_processos = 1

    
    tamanho_chunk = math.ceil(len(numeros) / num_processos)
    chunks = [numeros[i:i + tamanho_chunk] for i in range(0, len(numeros), tamanho_chunk)]

    
    with Pool(processes=num_processos) as pool:
        chunks_ordenados = pool.map(sorted, chunks)

    numeros_ordenados_iter = heapq.merge(*chunks_ordenados)

    tempo_decorrido = time.time() - inicio

    print("Salvando números ordenados em arquivo separado...")
    with open("numeros_ordenados.txt", "w") as arquivo:
        for numero in numeros_ordenados_iter:
            arquivo.write(f"{numero}\n")

    print(f"Números ordenados salvos em 'numeros_ordenados.txt' com sucesso!")
    print(f"Tempo de ordenação: {tempo_decorrido:.2f} segundos")

def mover_para_area_trabalho():
    print("Movendo arquivos para Área de Trabalho...")
    
    # Enviar para area de trab
    desktop = str(Path.home() / "Desktop")
    
    # Move numeros_grandes.txt
